...
public class gra {
        ...
        confirmButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                graghButton.addActionListener(new ActionListener() {
                    ...
                    public void actionPerformed(ActionEvent e) {
                        // 从某个地址读取图像
                        ImageIcon icon = new ImageIcon("./demo/src/main/java/com/example/graph/graph.png");

                        // 显示图像对话框
                        // JOptionPane.showMessageDialog(null, icon, "查看有向图",
                ...
                bridgeButton.addActionListener(new ActionListener() {
                    ...
                    public void actionPerformed(ActionEvent e) {
                        ...
                        if (word1 != null && !word1.isEmpty()) {
                            ...
                            // 弹出输入框让用户输入第二个单词
                            String word2 = JOptionPane.showInputDialog(null, "Enter the second word:",
                                    "输入单词2", JOptionPane.PLAIN_MESSAGE);
                            if (word2 != null && !word2.isEmpty()) {
                                ...
                                // 弹出消息对话框显示结果
                                JOptionPane.showMessageDialog(null, message, "查询结果",
                ...
                newTextButton.addActionListener(new ActionListener() {
                    ...
                    public void actionPerformed(ActionEvent e) {
                        ...
                        if (text != null && !text.isEmpty()) {
                            ...
                            // 弹出消息对话框显示生成的新文本
                            JOptionPane.showMessageDialog(null, newtext, "生成的新文本",
                                    JOptionPane.INFORMATION_MESSAGE);
                        }
                    }
                });
                ...
                shortestPathButton.addActionListener(new ActionListener() {
                    ...
                    public void actionPerformed(ActionEvent e) {
                        ...
                        if (input != null && !input.isEmpty()) {
                            ...
                            String result = calculateShortestPath(Graph, input);
                            // 弹窗显示结果
                            // JOptionPane.showMessageDialog(null, result, "计算最短路径结果",
                            // JOptionPane.PLAIN_MESSAGE);
                            // 创建 JTextArea 并设置结果
                            ...
                            JOptionPane.showMessageDialog(null, scrollPane,
                                    "计算最短路径结果", JOptionPane.PLAIN_MESSAGE);
                        }
                    }
                });
                ...
                randomWalkButton.addActionListener(new ActionListener() {
                    ...
                    public void actionPerformed(ActionEvent e) {
                        ...
                        JOptionPane.showMessageDialog(null,
                                "If you want to stop the traversal, enter 'stop'",
                                "Random Walk", JOptionPane.INFORMATION_MESSAGE);

                        // 创建一个独立的线程来执行随机遍历
                        Thread randomWalkThread = new Thread(new Runnable() {
                            @Override
                            public void run() {
                        ...
                        stopButton.addActionListener(new ActionListener() {
                            ...
                            public void actionPerformed(ActionEvent e) {
                                st = true; // 中断线程
                                // randomWalkThread.interrupt(); // 中断随机遍历线程
    ...
    public static void showDirectedGraph(Map<String, Map<String, Integer>> Graph, String filename) {
        ...
        for (int i = 0; i < words.length - 1; i++) {
            ...
            Map<String, Integer> innerMap1 = Graph.get(wordA);
            if (innerMap1 != null) {
                // 如果已经存在 wordA -> wordB 的关系，则增加计数
                Integer count = innerMap1.get(wordB);
                if (count != null) {
                    innerMap1.put(wordB, count + 1);
                } else {
                    // 否则，将 wordA -> wordB 的关系存入内部 Map
                    innerMap1.put(wordB, 1);
                }
            } else {
                // 如果不存在 wordA -> wordB 的关系，则创建新的内部 Map 并存入关系
                Map<String, Integer> innerMap2 = new HashMap<>();
                innerMap2.put(wordB, 1);
                Graph.put(wordA, innerMap2);
            }
        }
        // 将该图显示出来
        // 将图结构转换为DOT语言
        ...
        // 将DOT语言保存为.dot文件
        saveDotFile(dotContent, "./demo/src/main/java/com/example/graph/graph.dot");
        // 使用Graphviz将DOT文件转换为PNG图像
        convertDotToPng("./demo/src/main/java/com/example/graph/graph.dot",
                "./demo/src/main/java/com/example/graph/graph.png");
    }

    /**
     * 将图结构转换为DOT语言.
     */
    public static String convertToDot(Map<String, Map<String, Integer>> graph,
            ...
            for (Map.Entry<String, Integer> edgeEntry : edges.entrySet()) {
                ...
                if (shortestPaths != null) {
                    for (int i = 0; i < shortestPaths.size() - 1; i++) {
                        if (sourceNode.equals(shortestPaths.get(i))
                                && targetNode.equals(shortestPaths.get(i + 1))) {
     ...
     * 查询桥接词.
     */
    public static String queryBridgeWords(Map<String, Map<String, Integer>> G, String word1, String word2) {
    ...
    public static String calcShortestPath(Map<String, Map<String, Integer>> Graph, String word1, String word2) {
        ...
        for (Map.Entry<String, Map<String, Integer>> entry : Graph.entrySet()) {
            ...
            Map<String, Integer> innerMap = entry.getValue();
            if (!distance.containsKey(outWord)) {
                distance.put(outWord, Integer.MAX_VALUE);
                // System.out.println(out_word);
            }
            for (String inWord : innerMap.keySet()) {
                if (!distance.containsKey(inWord)) {
                    distance.put(inWord, Integer.MAX_VALUE);
        ...
        // 将DOT语言保存为.dot文件
        saveDotFile(dotContent, "./demo/src/main/java/com/example/graph/graph_least_road_" + nums + ".dot");
        // 使用Graphviz将DOT文件转换为PNG图像
        convertDotToPng("./demo/src/main/java/com/example/graph/graph_least_road_" + nums + ".dot",
                "./demo/src/main/java/com/example/graph/graph_least_road_"
                        + nums + ".png");
    ...
    /**
     * 函数：创建一个方法来处理计算最短路径的逻辑.
     */
    public static String calculateShortestPath(Map<String, Map<String, Integer>> Graph, String input) {
        ...
        if (startEnd.length == 2) {
            ...
            String shortestPath = calcShortestPath(Graph, start, end);
            ...
            for (Map.Entry<String, Map<String, Integer>> entry : Graph.entrySet()) {
                String outWord = entry.getKey();
                Map<String, Integer> innerMap = entry.getValue();
                boolean added1 = visited.add(outWord);
                if (added1) {
                    String end = outWord;
                    String shortestPath = calcShortestPath(Graph, start, end);
                    nums++;
                    if (shortestPath != null) {
                        ...
                        builder.append("\n最短路径长度为：").append(roadLength).append("\n");
                    } else {
                        builder.append("找不到从 ").append(start).append(" 到 ").append(end).append(" 的路径。\n");
                    }
                }
                for (String inWord : innerMap.keySet()) {
                    boolean added = visited.add(inWord);
                    if (added) {
                        String end = inWord;
                        String shortestPath = calcShortestPath(Graph, start, end);
                        nums++;
